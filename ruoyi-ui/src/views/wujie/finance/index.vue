<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="财务类别" prop="state" label-width="100px">
      ...
      <el-form-item label="财务状态" prop="state">
        <el-select v-model="queryParams.state" clearable>
          <el-option :value=0 label="未收/付款"></el-option>
          <el-option :value=1 label="已收/付款"></el-option>
          <el-option :value=2 label="已开票"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item class="newline">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['wujie:finance:add']">新增</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
            v-hasPermi="['wujie:finance:edit']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
            v-hasPermi="['wujie:finance:remove']">删除</el-button>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="付款/收款方" prop="paypeople">
        ...
        <el-form-item label="财务状态" prop="state">
          <el-select v-model="form.state" clearable>
            <el-option v-for="item in MaptoList(stateEnum)" :key="item.key" :label="item.label"
...
<script>
import { listFinance, getFinance, delFinance, addFinance, updateFinance } from "@/api/wujie/finance";
import { addFile } from '@/api/wujie/file';
import { getToken } from "@/utils/auth";
import {stateEnum, typeEnum} from './maps'
...
export default {
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('wujie/finance/export', {
        ...this.queryParams
      }, `finance_${new Date().getTime()}.xlsx`)
    },
    SetUploadcolumn(column) {
      this.upload.financeId = column.id;
    },
    ...
    handleFileSuccess(response, file, fileList) {
      ...
      let queryparams = {
        financeId: this.upload.financeId,
        useType: 1,
    ...
    handleDetail(row) {
      this.$router.push({ path: "finance/detail", query: { id: row.id } });
    },
    MaptoList(mp) {
      let list = [];
